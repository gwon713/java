...
public class Service {
	...
	public Service(){
		this.path = "src/p0427Memo/memo";
		dir = new File(path);
	}
	//디렉토리 생성
	public boolean mkDir(){
		if(!dir.exists()){//디렉토리 이름 중복체크
			dir.mkdir();
	...
	public String mkFileName(Scanner sc){
		...
		do{
			...
			fName = sc.next();//쓰기에서 사용할 파일명 입력
			for(i=0; i<files.length; i++){
				if(fName.equals(files[i])){//중복검사
	...
	public void read(Scanner sc){
		String[] files = dir.list();//읽은 파일 목록 저장
		int i;
		if(files.length <= 0){
			System.out.println("읽은 파일이 없다.");
			return;
		}
		...
		try {
			...
			while((i=br.read())!=-1){//파일의 끝을 만날때까지 1바이트씩 읽어서 i에 저장
